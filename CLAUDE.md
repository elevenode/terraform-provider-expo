...
- **Push to main** (release.yml): Verify gate (`gofmt`/`go build`/`go vet`/unit tests) → creates semver release → builds with GoReleaser
- Docs in `docs/` are auto-generated by `tfplugindocs` -- do not edit manually. CI no longer generates them; **Claude must run `tfplugindocs` locally and commit the resulting `docs/` changes whenever a change touches `schema.go`, `examples/`, or resource/data-source descriptions, before pushing.**
