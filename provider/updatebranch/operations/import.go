...
// Import takes an "<app_id>/<branch_name>" address, since an update branch is
...
func Import(ctx context.Context, d *schema.ResourceData, m any) ([]*schema.ResourceData, error) {
	...
	if !found || appId == "" || name == "" {
		return nil, fmt.Errorf("invalid import id %q, expected \"<app_id>/<branch_name>\"", d.Id())
	}

	data, err := client.UpdateBranch.GetByName(eas.GetByNameUpdateBranchData{
		AppId: appId,
		Name:  name,
	})
	if err != nil {
